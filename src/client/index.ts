    ...
    function TaskPanel(props) {
      ...
      const jumpSession = (p) => {
        ...
        openAgentConversation(p.sessionId, p.parentSessionId).then((ok) => {
          if (!ok) setConvReq({ id: p.id, title: p.title, sessionId: p.sessionId })
        })
      }
      ...
      return h('div', { className: 'dtp-root' },
        h('div', { className: 'dtp-header' },
          ...
          h('div', { className: 'dtp-header-stats' },
            h('span', { className: 'dtp-stat' }, '队列 ' + queued),
            h('span', { className: 'dtp-stat' }, '执行 ' + executing),
            h('span', { className: 'dtp-stat' }, '复核 ' + reviewing),
        ...
        h('div', { className: 'dtp-board dtp-scroll' },
          columns.map((col) => {
            ...
            return h('div', { className: 'dtp-col', key: col.stage, style: { '--dtp-accent': col.color } },
    ...
    function Card(props) {
      ...
      return h('div', { className: 'dtp-card' },
        ...
        h('div', { className: 'dtp-card-meta' },
          ...
          h('span', { title: req.autoReview === false ? '执行完成后不启动复核 agent' : '执行完成后自动启动复核 agent', style: { color: req.autoReview === false ? 'var(--dsw-alias-state-warn-primary)' : 'var(--dsw-alias-link)' } }, req.autoReview === false ? '免复核' : '自动复核'),
          req.contextAnchors && req.contextAnchors.length ? h('span', { title: '已关联历史会话', style: { color: 'var(--dsw-alias-state-success-primary)' } }, '会话 ' + req.contextAnchors.length) : null,
          req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'var(--dtp-muted)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '目录 ' + req.workdir) : h('span', { style: { color: 'var(--dsw-alias-state-warn-primary)' } }, '未绑定目录'),
          req.scheduledAt ? h('span', { title: '计划执行时间', style: { color: req.scheduledAt > Date.now() ? 'var(--dsw-alias-link)' : 'var(--dsw-alias-state-success-primary)' } }, '定时 ' + formatScheduledAt(req.scheduledAt)) : null,
          req.reworkCount ? h('span', { style: { color: 'var(--dsw-alias-state-warn-primary)' } }, '返工 ' + req.reworkCount) : null,
        ),
        progressBlock,
        reviewBlock,
        deliverableBlock,
    ...
    function RequirementForm(props) {
      ...
      return h('div', { className: 'dtp-modal-backdrop', onClick: onClose },
        h('div', { className: 'dtp-modal', onClick: (e) => e.stopPropagation() },
          ...
          !isEdit && !workdir
            ? h('div', { className: 'dtp-field', style: { background: 'var(--dsw-alias-state-warn-tertiary)', border: '1px solid var(--dtp-hairline)', borderRadius: 9, padding: '7px 10px', color: 'var(--dsw-alias-state-warn-primary)', fontSize: 11 } },
      ...
      return h('div', { className: 'dtp-modal-backdrop', onClick: onClose },
        h('div', { className: 'dtp-modal', style: { width: 560 }, onClick: (e) => e.stopPropagation() },
          ...
          h('div', { className: 'dtp-dirbrowser' },
            ...
            error ? h('div', { style: { fontSize: 11, color: 'var(--dsw-alias-state-error-primary)', padding: '6px 10px', background: 'var(--dsw-alias-state-error-tertiary)', border: '1px solid var(--dtp-hairline)', borderRadius: 9 } }, error) : null,
            mode !== 'native'
