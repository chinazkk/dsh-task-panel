  ...
  apply(ctx) {
    ...
    function buildPrompt(req, execDir) {
      ...
      if (req.reworkCount > 0) {
        ...
        if (earlier.length) {
          prevRoundLines.push('历史轮次：' + earlier.map((e, i) =>
            '第' + (e.round || (i + 1)) + '轮=' + clipText(e.summary || '（无摘要）', 80)).join('；'))
        }
        ...
        if (changed.length) prevRoundLines.push('改动文件（前 20 个）：' + changed.slice(0, 20).join(', ') + (changed.length > 20 ? '，等 ' + changed.length + ' 个' : ''))
        if (prev.testCommand) prevRoundLines.push('验证命令：' + prev.testCommand)
        if (prev.testResult) prevRoundLines.push('验证结果：' + clipText(prev.testResult, 500))
        if (prev.blocker) prevRoundLines.push('执行阻塞：' + clipText(prev.blocker, 240))
        if (prev.sessionId) prevRoundLines.push('原会话：sessionId=' + prev.sessionId + '（需要更多过程细节时可按此查询原会话）')
