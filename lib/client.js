    ...
    `);
					...
					function Card(props) {
						...
						return h("div", { className: "dtp-card" }, h("div", { className: "dtp-card-title" }, h("span", { className: "dtp-pri dtp-pri-" + pri }, pri), " ", req.title), h("div", { className: "dtp-card-meta" }, h("span", null, req.id), h("span", null, "要素 " + req.elementCount), h("span", null, "验收 " + req.criterionCount), h("span", {
							...
							style: { color: req.autoReview === false ? "rgba(236,230,213,.92)" : "rgba(217,227,242,.90)" }
						}, req.autoReview === false ? "免复核" : "自动复核"), req.contextAnchors && req.contextAnchors.length ? h("span", {
							title: "已关联历史会话",
							style: { color: "rgba(224,238,232,.90)" }
						...
						}, "目录 " + req.workdir) : h("span", { style: { color: "rgba(236,230,213,.92)" } }, "未绑定目录"), req.scheduledAt ? h("span", {
							title: "计划执行时间",
							style: { color: req.scheduledAt > Date.now() ? "rgba(217,227,242,.90)" : "rgba(224,238,232,.90)" }
						}, "定时 " + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h("span", { style: { color: "rgba(236,230,213,.92)" } }, "返工 " + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
					}
					function RequirementForm(props) {
						...
						return h("div", {
							...
							className: "dtp-field",
							style: {
								background: "rgba(203,193,170,.10)",
								border: "1px solid rgba(226,232,240,.14)",
								borderRadius: 9,
								padding: "7px 10px",
								color: "rgba(236,230,213,.92)",
								fontSize: 11
							}
					...
					function DirectoryPickerModal(props) {
						...
						return h("div", {
							...
							fontSize: 11,
							color: "rgba(239,218,221,.92)",
							padding: "6px 10px",
							background: "rgba(201,174,178,.10)",
