window.__ModuleLoader__.load({
    ...
    `);
					...
					function TaskPanel(props) {
						...
						return h("div", { className: "dtp-root" }, h("div", { className: "dtp-header" }, h("div", { className: "dtp-title" }, h("div", { className: "dtp-logo" }, "▦"), h("div", null, h("h1", null, "任务面板"), h("div", { className: "sub" }, total + " 条需求" + (executing ? " · " + executing + " 执行中" : "") + (reviewing ? " · " + reviewing + " 复核中" : "") + (accepting ? " · " + accepting + " 待验收" : "") + " · 队列在子 session 自动执行"))), h("button", {
					...
					function Card(props) {
						...
						else if (stage === "reviewing") actions = h("div", { className: "dtp-actions" }, h("span", { style: {
							fontSize: 11,
							color: "#c084fc",
							flexBasis: "100%"
						} }, h("span", {
						...
						if ((stage === "accepting" || stage === "accepted") && req.reviewCount) {
							...
							reviewBlock = h("div", {
								...
								title: "自动复核结论"
							}, h("span", { className: "lab" }, passed ? "复核✓" : "复核!"), h("span", null, text));
						}
						...
						return h("div", { className: "dtp-card" }, h("div", { className: "dtp-card-title" }, h("span", { className: "dtp-pri dtp-pri-" + pri }, pri), " ", req.title), h("div", { className: "dtp-card-meta" }, h("span", null, req.id), h("span", null, "要素 " + req.elementCount), h("span", null, "验收 " + req.criterionCount), h("span", {
							...
							style: { color: req.autoReview === false ? "#fbbf24" : "#a78bfa" }
						}, req.autoReview === false ? "免复核" : "自动复核"), req.workdir ? h("span", {
							title: "绑定工作目录",
							style: {
						...
						}, "📁 " + req.workdir) : h("span", { style: { color: "#fbbf24" } }, "⚠ 未绑定目录"), req.scheduledAt ? h("span", {
							title: "计划执行时间",
							style: { color: req.scheduledAt > Date.now() ? "#a78bfa" : "#34d399" }
						}, "⏱ " + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h("span", { style: { color: "#fbbf24" } }, "返工 " + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
					}
					function RequirementForm(props) {
						...
						}), h("span", { className: "txt" }, "自动复核", h("span", { className: "hint" }, autoReview ? "执行完成后启动复核 agent，再进入待验收。" : "执行完成后直接进入待验收，不启动复核 agent。")))), h("div", { className: "dtp-modal-actions" }, h("button", {
							className: "dtp-btn",
							onClick: onClose
						}, "取消"), h("button", {
							className: "dtp-btn primary",
