window.__ModuleLoader__.load({
    ...
    `);
					...
					function TaskPanel(props) {
						...
						const [progressReq, setProgressReq] = react.useState(null);
					...
					function Card(props) {
						...
						if (stage === "executing" && progress && Array.isArray(progress.recent) && progress.recent.length) {
							...
							progressBlock = h("div", { className: "dtp-progress" }, progress.workdir ? h("div", {
								className: "pmeta",
					...
					function ProgressModal(props) {
						...
						react.useEffect(() => {
							if (detail && detail.sessionId) {
								const t = ctx.interval(() => setElapsed(Date.now()), 1e3);
								return () => {
						...
						return h("div", {
							...
							style: { width: 680 },
							onClick: (e) => e.stopPropagation()
						}, h("h2", null, "执行进度 · " + req.title), h("div", { style: {
							fontSize: 11,
							color: "var(--dsw-alias-label-secondary, #9297a5)",
							marginBottom: 10,
							display: "flex",
							...
							color: "#fbbf24",
							fontWeight: 700
						} }, h("span", { className: "dtp-pulse" }), "子 agent 执行中"), detail && detail.sessionId ? h("span", null, "session: " + String(detail.sessionId).slice(0, 12) + "…") : null, h("span", null, "已运行 " + fmt(elapsed) + " · " + rows.length + " 条消息")), rows.length === 0 ? h("div", { className: "dtp-empty" }, h("span", { className: "dtp-spin" }), "等待子 agent 输出…") : h("div", {
							className: "dtp-transcript dtp-scroll",
							style: {
								maxHeight: "46vh",
								overflowY: "auto"
							}
							...
							key: i
						}, h("span", { className: "who" }, who[m.role] || m.role), m.text))), h("div", { className: "dtp-modal-actions" }, detail && detail.sessionId ? h("button", {
							className: "dtp-btn primary",
							onClick: () => {
								openAgentConversation(detail.sessionId, detail.parentSessionId).then((ok) => {
									if (!ok) onClose();
								});
							}
						}, "↗ 进入子代理会话") : null, h("button", {
							className: "dtp-btn",
							onClick: onClose
						}, "关闭"))));
					}
