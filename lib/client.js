window.__ModuleLoader__.load({
    ...
    `);
					...
					function TaskPanel(props) {
						...
						const columns = [
							...
							},
							{
								stage: "executing",
								title: "执行中",
								color: "rgba(196,181,143,.58)",
								count: byStage("executing").length
							},
							{
								stage: "reviewing",
								title: "自动复核",
								color: "rgba(184,196,214,.52)",
					...
					function Card(props) {
						...
						else if (stage === "executing") actions = h("div", { className: "dtp-actions" }, h("span", {
							className: "dtp-status-line",
							style: { color: "#e9dfc2" }
						...
						else if (stage === "reviewing") actions = h("div", { className: "dtp-actions" }, h("span", {
							className: "dtp-status-line",
							style: { color: "#d9e3f2" }
						}, h("span", {
							className: "dtp-pulse",
							style: { background: "#b8c4d6" }
						...
						return h("div", { className: "dtp-card" }, h("div", { className: "dtp-card-title" }, h("span", { className: "dtp-pri dtp-pri-" + pri }, pri), " ", req.title), h("div", { className: "dtp-card-meta" }, h("span", null, req.id), h("span", null, "要素 " + req.elementCount), h("span", null, "验收 " + req.criterionCount), h("span", {
							...
							style: { color: req.autoReview === false ? "#e9dfc2" : "#d9e3f2" }
						...
						}, "📁 " + req.workdir) : h("span", { style: { color: "#e9dfc2" } }, "⚠ 未绑定目录"), req.scheduledAt ? h("span", {
							title: "计划执行时间",
							style: { color: req.scheduledAt > Date.now() ? "#d9e3f2" : "#d7eee4" }
						}, "⏱ " + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h("span", { style: { color: "#e9dfc2" } }, "返工 " + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
					}
					function RequirementForm(props) {
						...
						return h("div", {
							...
							className: "dtp-field",
							style: {
								background: "rgba(196,181,143,.12)",
								border: "1px solid rgba(196,181,143,.30)",
								borderRadius: 10,
								padding: "7px 10px",
								color: "#e9dfc2",
								fontSize: 11
					...
					function DirectoryPickerModal(props) {
						...
						return h("div", {
							...
							color: "#ead5d8",
							padding: "6px 10px",
							background: "rgba(197,155,161,.10)",
							border: "1px solid rgba(197,155,161,.20)",
							borderRadius: 10
