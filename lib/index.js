...
const plugin = (() => {
    ...
    return {
        ...
        apply(ctx) {
            ...
            // [p8] 无可用 agent 时挂起的队列：任一 agent 就绪（用户开启对话创建根 agent）即续跑派发，
            ...
            function resolveDataBaseDir() {
                // 1) [p7 已固定] 数据位置不再跟随 lastWorkdir（防跨重启状态分叉/历史丢失），直接走 workspaceRoot
            ...
            // [p8] 无可用父级 agent 时挂起等待：不吞任务、不伪造执行——
            ...
            async function pump() {
                ...
                busy = true;
                persistState();
                // [p8] 先探测父级 agent 可用性，再开始执行：无 agent 时挂起放回队首。
