        ...
        apply(ctx) {
            ...
            function resolveDataBaseDir() {
                ...
                if (typeof lastWorkdir === 'string' && lastWorkdir.trim())
                    return normalizePanelWorkdir(lastWorkdir);
            ...
            function buildReviewPrompt(req) {
                ...
                const lines = [
                    ...
                    '执行摘要：' + (exec.summary || '（无）'),
                    exec.changedFiles && exec.changedFiles.length ? '改动文件：' + exec.changedFiles.join(', ') : '',
                    exec.testCommand ? '验证命令：' + exec.testCommand : '',
                    exec.testResult ? '验证结果：' + exec.testResult : '',
                    exec.blocker ? '执行阻塞：' + exec.blocker : '',
                    '请对照验收要素、代码质量、测试证据和潜在遗漏做复核。不要修改文件，只做检查。',
