        ...
        apply(ctx) {
            ...
            function captureSignalCtor(signal) {
                if (!AbortSignalCtor && signal && typeof signal === 'object' && signal.constructor) {
                    try {
                        AbortSignalCtor = signal.constructor;
                    }
                    catch (e) { /* noop */ }
                }
            }
            ctx.on('agent/pre-step', (payload, next) => {
                try {
                    if (payload && payload.signal)
                        captureSignalCtor(payload.signal);
                }
                catch (e) { /* noop */ }
                return next();
            });
            ...
            function addEvent(req, type, message, meta) {
                ...
                if (req.events.length > 120)
                    req.events = req.events.slice(-120);
            }
            function ensureRequirementShape(req) {
                ...
                for (const exec of req.executions) {
                    ...
                    if (isErrorStopReason(exec.stopReason)) {
                        ...
                        exec.done = false;
                        if (!exec.summary || /^执行完成（stopReason=/.test(exec.summary))
                            exec.summary = '执行失败：' + diagnostic;
                        if (!exec.blocker)
                            exec.blocker = diagnostic;
                    }
                }
                ...
                for (const review of req.reviews) {
                    ...
                    if (isErrorStopReason(review.stopReason)) {
                        ...
                        review.passed = false;
                        if (!review.verdict)
                            review.verdict = '自动复核失败：' + diagnostic;
                        if (!Array.isArray(review.issues) || !review.issues.length)
                            review.issues = [diagnostic];
                    }
                }
            ...
            async function suggestContextSessions(input, limit) {
                ...
                if (!query || !sessionQuery || typeof sessionQuery.searchSessions !== 'function')
                    return [];
                try {
                    ...
                    return items.map(contextAnchorFromHit).filter((anchor) => {
                        if (!anchor || seen.has(anchor.sessionId))
                            return false;
            ...
            function resolveDataTarget() {
                if (dataTarget && dataBaseDir === resolveDataBaseDir())
                    return Promise.resolve(dataTarget);
                if (dataTargetPromise)
            ...
            async function loadState() {
                try {
                    ...
                    try {
                        ...
                        if (oldBase)
                            candidates.push(oldBase + '/.dsh-task-panel/requirements.json');
                    }
                    catch (e) { /* noop */ }
                    ...
                    if (parsed && typeof parsed.lastWorkdir === 'string' && parsed.lastWorkdir) {
                        ...
                        if (lastWorkdir !== resolveDataBaseDir()) {
                            ...
                            if (newTarget) {
                                try {
                                    await fs.writeText(newTarget, text, undefined, undefined, resolveSessionPolicy());
                                    fromOld = true;
                                }
                                catch (e) { /* noop */ }
                            }
                    ...
                    if (final.requirements && typeof final.requirements === 'object') {
                        for (const k of Object.keys(final.requirements))
                            requirements[k] = ensureRequirementShape(final.requirements[k]);
                    }
                    if (Array.isArray(final.backlog)) {
                        backlog.length = 0;
                        backlog.push(...final.backlog);
                    }
                    if (Array.isArray(final.execQueue)) {
                        execQueue.length = 0;
                        execQueue.push(...final.execQueue);
                    }
                    if (typeof final.lastWorkdir === 'string' && final.lastWorkdir)
                        lastWorkdir = normalizePanelWorkdir(final.lastWorkdir);
                }
                catch (e) { /* 首次运行没有数据 */ }
            }
            ...
            function decomposeElements(input) {
                ...
                const cats = [];
                if (/api|接口|http|rest|后端|服务|endpoint/i.test(desc))
                    cats.push('api');
                if (/ui|界面|页面|前端|组件|web|交互/i.test(desc))
                    cats.push('ui');
                if (/test|测试|单测|集成|用例/i.test(desc))
                    cats.push('test');
                if (/config|配置|环境/i.test(desc))
                    cats.push('config');
                if (cats.length === 0)
                    cats.push('feature');
                return cats.map((c, i) => ({
            ...
            function create(input) {
                ...
                const acceptanceCriteria = input.acceptanceCriteria && input.acceptanceCriteria.length
            ...
            function recallFromExec(id) {
                ...
                if (ei >= 0) {
                    execQueue.splice(ei, 1);
                    if (backlog.indexOf(id) < 0)
                        backlog.push(id);
                    const req = requirements[id];
                    if (req) {
                        req.stage = 'backlog';
                        req.updatedAt = Date.now();
                        addEvent(req, 'recalled', '已从执行队列撤回');
                    }
                    persistState();
                }
            }
            ...
            function completeExecution(id, summary, meta) {
                ...
                req.updatedAt = Date.now();
                addEvent(req, failed ? 'execution-failed' : 'execution-completed', failed ? '执行未正常完成，已进入待验收'
            ...
            function deferExecutionForAgent(id) {
                ...
                req.stage = 'queued';
                if (execQueue.indexOf(id) < 0)
                    execQueue.unshift(id); // 放回队首，保持原排队次序
                req.updatedAt = Date.now();
                ...
                panelCreating = (async () => {
                    try {
                        ...
                        try {
                            ...
                            if (sel) {
                                if (sel.provider)
                                    agentOptions.provider = sel.provider;
                                if (sel.model)
                                    agentOptions.model = sel.model;
                            }
                        }
                        catch (e) {
                            panelDiag = 'model:' + (e && e.message ? e.message : String(e));
                        }
            ...
            function buildPrompt(req, execDir) {
                ...
                const anchorLines = anchors.length
                    ? ['关联历史会话（可作为上下文参考，必要时按 sessionId 打开/查询原会话）：'].concat(anchors.map((a, i) => '[' + (i + 1) + '] sessionId=' + a.sessionId +
                        (a.title ? ' 标题=' + a.title : '') +
                        (a.cwd ? ' cwd=' + a.cwd : '') +
                        (a.snippet ? '\n    摘要片段：' + a.snippet : '')))
                    : [];
                // 返工上下文注入：把上一轮执行情况（产物摘要/改动文件/验证结果/原会话）写进返工提示词，
                // 让返工 agent 知道此前进度，在现有产物上迭代而非从零重做。
                // 取「最后一个有内容的执行」——startExecution 会先 push 空的当前轮记录，数组末位是本轮。
                const prevRoundLines = [];
                if (req.reworkCount > 0) {
                    const execs = (Array.isArray(req.executions) ? req.executions : [])
                        .filter((e) => e && (e.summary || e.completedAt));
                    const prev = execs[execs.length - 1] || {};
                    const earlier = execs.slice(0, -1);
                    if (earlier.length) {
                        prevRoundLines.push('历史轮次：' + earlier.map((e, i) => '第' + (e.round || (i + 1)) + '轮=' + clipText(e.summary || '（无摘要）', 80)).join('；'));
                    }
                    prevRoundLines.push('── 上一轮执行情况（第 ' + (prev.round || req.reworkCount) + ' 轮）──');
                    prevRoundLines.push('产物摘要：' + (clipText(prev.summary, 400) || '（无）'));
                    const changed = Array.isArray(prev.changedFiles) ? prev.changedFiles.filter(Boolean) : [];
                    if (changed.length)
                        prevRoundLines.push('改动文件（前 20 个）：' + changed.slice(0, 20).join(', ') + (changed.length > 20 ? '，等 ' + changed.length + ' 个' : ''));
                    if (prev.testCommand)
                        prevRoundLines.push('验证命令：' + prev.testCommand);
                    if (prev.testResult)
                        prevRoundLines.push('验证结果：' + clipText(prev.testResult, 500));
                    if (prev.blocker)
                        prevRoundLines.push('执行阻塞：' + clipText(prev.blocker, 240));
                    if (prev.sessionId)
                        prevRoundLines.push('原会话：sessionId=' + prev.sessionId + '（需要更多过程细节时可按此查询原会话）');
                    prevRoundLines.push('请先查看上一轮产物与改动文件，在现有基础上针对返工原因修复；除非返工原因表明方向性错误，否则不要从零重做。');
                }
                const lines = [
                ...
                try {
                    ...
                    for (const ev of events) {
                        ...
                        if (ev.type === 'user/message' && ev.data && Array.isArray(ev.data.content)) {
                            ...
                            if (text)
                                out.push({ role: 'user', text: text.slice(0, 2000), time });
                        }
                            ...
                            if (text)
                                out.push({ role: 'assistant', text: text.slice(0, 4000), time });
                        }
            ...
            async function runReview(id, parent) {
                ...
                if (userStopped.has(id)) {
                    ...
                    req.stage = target;
                    if (target === 'backlog' && backlog.indexOf(id) < 0)
                        backlog.push(id);
                    req.updatedAt = Date.now();
            ...
            handle('progress', async (a) => {
                ...
                for (const k of Object.keys(requirements)) {
                    ...
                    let recent = [];
                    if (sessionId)
                        recent = await captureTranscript(sessionId);
                    out.push({
                        id: r.id,
                        title: r.title,
                        stage: r.stage,
            ...
            handle('browse-dir', async (a) => {
                ...
                if (!directoryPicker)
                    return { ok: false, error: 'directoryPicker 不可用' };
                try {
            ...
            registerTool(defineTool({
                ...
                async execute(args) {
                    ...
                    if (!r)
                        return '未找到需求 ' + args.requirementId;
                    const lines = [
            ...
            registerTool(defineTool({
                ...
                async execute(args) {
                    const req = requirements[args.requirementId];
                    if (!req)
                        return '需求不存在：' + args.requirementId;
            ...
            if (systemPrompt && typeof systemPrompt.section === 'function') {
                try {
                    ...
                    console.error('system prompt section failed', e);
                }
            }
            ...
            ctx.effect(() => () => {
                ...
                if (panelHandle) {
                    try {
                        void panelHandle.dispose().catch(() => { });
                    }
                    catch (e) { /* noop */ }
                    panelHandle = null;
                    panelAgent = null;
