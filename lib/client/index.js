...
const plugin = (() => {
    ...
    `);
            ...
            function TaskPanel(props) {
                ...
                return h('div', { className: 'dtp-root' }, h('div', { className: 'dtp-header' }, h('div', { className: 'dtp-title' }, h('div', { className: 'dtp-logo' }, '▦'), h('div', null, h('h1', null, '任务面板'), h('div', { className: 'sub' }, total + ' 条需求' + (executing ? ' · ' + executing + ' 执行中' : '') + (accepting ? ' · ' + accepting + ' 待验收' : '') + ' · 队列在子 session 自动执行'))), h('button', { className: 'dtp-btn primary', onClick: () => setFormReq({ mode: 'create' }) }, '＋ 新建需求')), h('div', { className: 'dtp-board dtp-scroll' }, columns.map((col) => h('div', { className: 'dtp-col', key: col.stage, style: { borderTop: '3px solid ' + col.color } }, h('div', { className: 'dtp-col-head' }, h('span', { className: 'dtp-dot', style: { color: col.color, background: col.color } }), h('span', { className: 'name' }, col.title), h('span', { className: 'dtp-count' }, String(col.count))), h('div', { className: 'dtp-col-body dtp-scroll' }, byStage(col.stage).length === 0
                    ? h('div', { className: 'dtp-empty' }, '— 暂无需求 —')
                    : byStage(col.stage).map((r) => h(Card, {
                        key: r.id,
                        req: r,
                        stage: col.stage,
                ...
                }) : null, progressReq ? h(ProgressModal, {
                    req: progressReq,
            ...
            function Card(props) {
                ...
                if (stage === 'backlog') {
                    ...
                    actions = h('div', { className: 'dtp-actions' }, h('span', { style: { fontSize: 11, color: '#fbbf24', flexBasis: '100%' } }, h('span', { className: 'dtp-pulse' }), '子 agent 执行中' + (progress && progress.sessionId ? ' · ' + String(progress.sessionId).slice(0, 8) : '') + (progress ? ' · ' + Math.round((progress.elapsedMs || 0) / 1000) + 's' : '')), h('button', { className: 'dtp-btn small primary', onClick: onShowProgress }, '▶ 查看进度'), h('button', { className: 'dtp-btn small', onClick: onPause }, '⏸ 暂停'), h('button', { className: 'dtp-btn small danger', onClick: onStop }, '⏹ 停止'));
                }
                ...
                if (stage === 'executing' && progress && Array.isArray(progress.recent) && progress.recent.length) {
                    ...
                    progressBlock = h('div', { className: 'dtp-progress' }, progress.workdir
                ...
                if ((stage === 'accepting' || stage === 'accepted') && req.deliverable) {
                    if (deliverableOpen) {
                        deliverableBlock = h('div', { className: 'dtp-deliverable', title: '验收产物（一句话）' }, h('span', { className: 'lab' }, '产物'), req.deliverable);
                    }
                    else {
                ...
                return h('div', { className: 'dtp-card' }, h('div', { className: 'dtp-card-title' }, h('span', { className: 'dtp-pri dtp-pri-' + pri }, pri), ' ', req.title), h('div', { className: 'dtp-card-meta' }, h('span', null, req.id), h('span', null, '要素 ' + req.elementCount), h('span', null, '验收 ' + req.criterionCount), req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'var(--dsw-alias-label-secondary, #9297a5)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '📁 ' + req.workdir) : h('span', { style: { color: '#fbbf24' } }, '⚠ 未绑定目录'), req.reworkCount ? h('span', { style: { color: '#fbbf24' } }, '返工 ' + req.reworkCount) : null), progressBlock, deliverableBlock, actions);
            }
            // ── 新建 / 编辑表单 ──
            function RequirementForm(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', onClick: (e) => e.stopPropagation() }, h('h2', null, isEdit ? '编辑需求 ' + req.id : '新建需求'), !isEdit && !workdir
                    ? h('div', { className: 'dtp-field', style: { background: 'rgba(245,158,11,.12)', border: '1px solid rgba(245,158,11,.35)', borderRadius: 8, padding: '7px 10px', color: '#fbbf24', fontSize: 11 } }, '⚠ 尚未绑定工作目录，子 agent 执行时无法确定落盘位置，请选择或填写「绑定工作目录」。')
                    : null, h('div', { className: 'dtp-field' }, h('label', null, '标题 *'), h('input', { value: title, onChange: (e) => setTitle(e.target.value), placeholder: '一句话描述需求' })), h('div', { className: 'dtp-field' }, h('label', null, '详细描述'), h('textarea', { value: description, onChange: (e) => setDescription(e.target.value), rows: 3, placeholder: '背景 / 目标 / 约束' })), h('div', { className: 'dtp-field' }, h('label', null, '优先级'), h('select', { value: priority, onChange: (e) => setPriority(e.target.value) }, ['critical', 'high', 'medium', 'low'].map((p) => h('option', { key: p, value: p }, p)))), h('div', { className: 'dtp-field' }, h('label', null, '绑定工作目录 *'), h('div', { className: 'dtp-workdir-row' }, h('input', { value: workdir, onChange: (e) => setWorkdir(e.target.value), placeholder: '如: /Users/xxx/project（子 agent 在此目录执行）' }), h('button', { className: 'dtp-btn', onClick: () => setDirPickerOpen(true) }, '📁 浏览…'))), h('div', { className: 'dtp-field' }, h('label', null, '涉及范围（逗号分隔）'), h('input', { value: scope, onChange: (e) => setScope(e.target.value), placeholder: '如: src/, docs/' })), h('div', { className: 'dtp-field' }, h('label', null, '执行命令（可选）'), h('input', { value: command, onChange: (e) => setCommand(e.target.value), placeholder: '如: npm test' })), h('div', { className: 'dtp-modal-actions' }, h('button', { className: 'dtp-btn', onClick: onClose }, '取消'), h('button', { className: 'dtp-btn primary', onClick: save, disabled: busy }, busy ? '保存中…' : '保存'))), dirPickerOpen ? h(DirectoryPickerModal, {
                    initial: workdir,
            ...
            function DirectoryPickerModal(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', style: { width: 560 }, onClick: (e) => e.stopPropagation() }, h('h2', null, '选择工作目录'), h('div', { className: 'dtp-dirbrowser' }, h('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } }, mode === 'native'
                    ...
                    : h('button', { className: 'dtp-btn', onClick: () => openDir(initial || undefined), disabled: loading }, '↻ 刷新'), h('button', { className: 'dtp-btn', onClick: () => openDir(undefined) }, '🏠 主目录')), error ? h('div', { style: { fontSize: 11, color: '#f87171', padding: '6px 10px', background: 'rgba(248,81,73,.08)', borderRadius: 6 } }, '⚠ ' + error) : null, mode !== 'native'
                    ...
                    ? h('div', { className: 'dtp-direntries' }, loading
                        ...
                        : entries.length === 0
                            ...
                            : entries.map((e, i) => h('div', { className: 'dtp-direntry', key: i, onClick: () => openDir(e.path) }, h('span', { className: 'ic' }, e.hidden ? '📂' : '📁'), h('span', { style: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, e.name), h('button', { className: 'dtp-btn small dtp-dirselect', onClick: (ev) => { ev.stopPropagation(); onPick(e.path); } }, '选用'))))
            ...
            function ProgressModal(props) {
                ...
                React.useEffect(() => {
                    ...
                    load();
                    const disposer = ctx.interval(() => load(), 1500);
                    return () => { cancelled = true; if (typeof disposer === 'function')
                        disposer(); };
                }, [req.id]);
                React.useEffect(() => {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', style: { width: 680 }, onClick: (e) => e.stopPropagation() }, h('h2', null, '执行进度 · ' + req.title), h('div', { style: { fontSize: 11, color: 'var(--dsw-alias-label-secondary, #9297a5)', marginBottom: 10, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' } }, h('span', { style: { color: '#fbbf24', fontWeight: 700 } }, h('span', { className: 'dtp-pulse' }), '子 agent 执行中'), detail && detail.sessionId ? h('span', null, 'session: ' + String(detail.sessionId).slice(0, 12) + '…') : null, h('span', null, '已运行 ' + fmt(elapsed) + ' · ' + rows.length + ' 条消息')), rows.length === 0
                    ? h('div', { className: 'dtp-empty' }, h('span', { className: 'dtp-spin' }), '等待子 agent 输出…')
                    : h('div', { className: 'dtp-transcript dtp-scroll', style: { maxHeight: '46vh', overflowY: 'auto' } }, rows.map((m, i) => h('div', { className: 'dtp-msg ' + m.role, key: i }, h('span', { className: 'who' }, who[m.role] || m.role), m.text))), h('div', { className: 'dtp-modal-actions' }, detail && detail.sessionId
                    ? h('button', { className: 'dtp-btn primary', onClick: () => { openAgentConversation(detail.sessionId, detail.parentSessionId).then((ok) => { if (!ok)
                            onClose(); }); } }, '↗ 进入子代理会话')
                    : null, h('button', { className: 'dtp-btn', onClick: onClose }, '关闭'))));
            }
