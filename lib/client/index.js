...
const plugin = (() => {
    ...
    `);
            ...
            function TaskPanel(props) {
                ...
                const columns = [
                    { stage: 'backlog', title: '需求队列', color: 'var(--dtp-faint)', count: byStage('backlog').length },
                    { stage: 'queued', title: '执行队列', color: 'var(--dtp-faint)', count: byStage('queued').length },
                    { stage: 'executing', title: '执行中', color: 'var(--dsw-alias-state-warn-primary)', count: byStage('executing').length },
                    { stage: 'reviewing', title: '自动复核', color: 'var(--dsw-alias-label-tertiary)', count: byStage('reviewing').length },
                    { stage: 'paused', title: '已暂停', color: 'var(--dtp-faint)', count: byStage('paused').length },
                    { stage: 'accepting', title: '待验收', color: 'var(--dtp-ok)', count: byStage('accepting').length },
                    { stage: 'accepted', title: '验收完成', color: 'var(--dtp-faint)', count: byStage('accepted').length },
                ];
            ...
            function Card(props) {
                ...
                return h('div', { className: 'dtp-card' }, h('div', { className: 'dtp-card-title' }, h('span', { className: 'dtp-pri dtp-pri-' + pri }, pri), ' ', req.title), h('div', { className: 'dtp-card-meta' }, h('span', null, req.id), h('span', null, '要素 ' + req.elementCount), h('span', null, '验收 ' + req.criterionCount), h('span', { title: req.autoReview === false ? '执行完成后不启动复核 agent' : '执行完成后自动启动复核 agent', style: { color: req.autoReview === false ? 'var(--dsw-alias-state-warn-primary)' : 'var(--dsw-alias-link)' } }, req.autoReview === false ? '免复核' : '自动复核'), req.contextAnchors && req.contextAnchors.length ? h('span', { title: '已关联历史会话', style: { color: 'var(--dsw-alias-state-success-primary)' } }, '会话 ' + req.contextAnchors.length) : null, req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'var(--dtp-muted)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '目录 ' + req.workdir) : h('span', { style: { color: 'var(--dsw-alias-state-warn-primary)' } }, '未绑定目录'), req.scheduledAt ? h('span', { title: '计划执行时间', style: { color: req.scheduledAt > Date.now() ? 'var(--dsw-alias-link)' : 'var(--dsw-alias-state-success-primary)' } }, '定时 ' + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h('span', { style: { color: 'var(--dsw-alias-state-warn-primary)' } }, '返工 ' + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
            }
            // ── 新建 / 编辑表单 ──
            function RequirementForm(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', onClick: (e) => e.stopPropagation() }, h('h2', null, isEdit ? '编辑需求 ' + req.id : '新建需求'), !isEdit && !workdir
                    ? h('div', { className: 'dtp-field', style: { background: 'var(--dsw-alias-state-warn-tertiary)', border: '1px solid var(--dtp-hairline)', borderRadius: 9, padding: '7px 10px', color: 'var(--dsw-alias-state-warn-primary)', fontSize: 11 } }, '尚未绑定工作目录，子 agent 执行时无法确定落盘位置，请选择或填写「绑定工作目录」。')
            ...
            function DirectoryPickerModal(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', style: { width: 560 }, onClick: (e) => e.stopPropagation() }, h('h2', null, '选择工作目录'), h('div', { className: 'dtp-dirbrowser' }, h('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } }, mode === 'native'
                    ...
                    : h('button', { className: 'dtp-btn', onClick: () => openDir(initial || undefined), disabled: loading }, '刷新'), h('button', { className: 'dtp-btn', onClick: () => openDir(undefined) }, '主目录')), error ? h('div', { style: { fontSize: 11, color: 'var(--dsw-alias-state-error-primary)', padding: '6px 10px', background: 'var(--dsw-alias-state-error-tertiary)', border: '1px solid var(--dtp-hairline)', borderRadius: 9 } }, error) : null, mode !== 'native'
...
;
(function () {
    // v2: --dtp-* 直接引用 DSH 官方 --dsw-alias-* 主题变量（body[data-ds-dark-theme] 自动切换深浅色）
    var css = ''
        + '.dtp-root{'
        + '--dtp-text:var(--dsw-alias-label-primary,rgba(248,250,252,.96));'
        + '--dtp-muted:var(--dsw-alias-label-secondary,rgba(203,213,225,.68));'
        + '--dtp-faint:var(--dsw-alias-label-tertiary,rgba(148,163,184,.48));'
        + '--dtp-hairline:var(--dsw-alias-border-l2,rgba(255,255,255,.14));'
        + '--dtp-glass-0:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.035));'
        + '--dtp-glass-1:var(--dsw-alias-interactive-bg-active,rgba(255,255,255,.07));'
        + '--dtp-glass-2:var(--dsw-alias-interactive-bg-hover-accent,rgba(255,255,255,.12));'
        + '--dtp-glass-3:rgba(30,40,60,.18);'
        + '--dtp-accent:var(--dsw-alias-link,#a9b8cc);'
        + '--dtp-accent-soft:var(--dsw-alias-state-business-tertiary,rgba(169,184,204,.18));'
        + '--dtp-ok:var(--dsw-alias-state-success-primary,#b8c9c1);'
        + '--dtp-ok-soft:var(--dsw-alias-state-success-tertiary,rgba(184,201,193,.10));'
        + '--dtp-warn:var(--dsw-alias-state-warn-primary,#cbc1aa);'
        + '--dtp-danger:var(--dsw-alias-state-error-primary,#c9aeb2);'
        + '--dtp-solid-a:var(--dsw-alias-bg-base,#090e14);'
        + '--dtp-solid-b:var(--dsw-alias-bg-layer-1,#090d13);'
        + '--dtp-solid-c:var(--dsw-alias-bg-layer-2,#111821);'
        + '--dtp-solid-d:var(--dsw-alias-bg-layer-1,#07140f);'
        + '--dtp-solid-e:var(--dsw-alias-bg-layer-2,#07111f);'
        + '--dtp-shadow-ink:rgba(30,40,60,.12);'
        + '--dtp-header-bg:var(--dsw-alias-bg-base,#090e14);'
        + '--dtp-col-bg:var(--dsw-alias-bg-base,#090e14);'
        + '--dtp-colhead-bg:var(--dsw-alias-bg-base,#090e14);'
        + '--dtp-btn-bg:transparent;'
        + '}'
        + 'body[data-ds-dark-theme] .dtp-root{--dtp-glass-3:rgba(255,255,255,.28);--dtp-shadow-ink:rgba(0,0,0,.22);--dtp-col-bg:linear-gradient(180deg,rgba(255,255,255,.10),rgba(255,255,255,.045));--dtp-colhead-bg:linear-gradient(180deg,rgba(12,18,27,.68),rgba(12,18,27,.28));--dtp-btn-bg:linear-gradient(180deg,rgba(255,255,255,.125),rgba(255,255,255,.055));}';
