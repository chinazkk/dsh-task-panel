    ...
    `);
            ...
            function TaskPanel(props) {
                ...
                return h('div', { className: 'dtp-root' }, h('div', { className: 'dtp-header' }, h('div', { className: 'dtp-title' }, h('div', { className: 'dtp-logo' }, '▦'), h('div', null, h('h1', null, '任务面板'), h('div', { className: 'sub' }, total + ' 条需求' + (executing ? ' · ' + executing + ' 执行中' : '') + (reviewing ? ' · ' + reviewing + ' 复核中' : '') + (accepting ? ' · ' + accepting + ' 待验收' : '') + ' · 队列在子 session 自动执行'))), h('button', { className: 'dtp-btn primary', onClick: () => setFormReq({ mode: 'create' }) }, '＋ 新建需求')), h('div', { className: 'dtp-board dtp-scroll' }, columns.map((col) => {
                    ...
                    return h('div', { className: 'dtp-col', key: col.stage, style: { borderTop: '3px solid ' + col.color } }, h('div', { className: 'dtp-col-head' }, h('span', { className: 'dtp-dot', style: { color: col.color, background: col.color } }), h('span', { className: 'name' }, col.title), h('span', { className: 'dtp-count' }, String(col.count))), h('div', { className: 'dtp-col-body dtp-scroll' }, items.length === 0
