    ...
    `);
            ...
            function TaskPanel(props) {
                ...
                const jumpSession = (p) => {
                    ...
                    openAgentConversation(p.sessionId, p.parentSessionId).then((ok) => {
                        if (!ok)
                            setToast('会话暂不可跳转，可打开「查看进度」查看实时对话');
                    });
                };
            ...
            function Card(props) {
                ...
                if (stage === 'backlog') {
                    ...
                    actions = h('div', { className: 'dtp-actions' }, h('span', { style: { fontSize: 11, color: '#fbbf24', flexBasis: '100%' } }, h('span', { className: 'dtp-pulse' }), '子 agent 执行中' + (progress && progress.sessionId ? ' · ' + String(progress.sessionId).slice(0, 8) : '') + (progress ? ' · ' + Math.round((progress.elapsedMs || 0) / 1000) + 's' : '')), h('button', {
                    ...
                    }, '↗ 会话'), h('button', { className: 'dtp-btn small primary', onClick: onShowProgress }, '▶ 查看进度'), h('button', { className: 'dtp-btn small', onClick: onPause }, '⏸ 暂停'), h('button', { className: 'dtp-btn small danger', onClick: onStop }, '⏹ 停止'));
                }
