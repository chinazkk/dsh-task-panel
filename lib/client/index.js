...
const plugin = (() => {
    ...
    return {
      ...
      .dtp-check { display: flex; align-items: flex-start; gap: 9px; padding: 9px 10px; background: rgba(255,255,255,.04); border: 1px solid var(--dsw-alias-border-l1, #2c3140); border-radius: 8px; }
      .dtp-check input { width: 16px; height: 16px; margin: 1px 0 0; flex: 0 0 auto; accent-color: #3b82f6; }
      .dtp-check .txt { flex: 1; min-width: 0; color: var(--dsw-alias-label-primary, #ececf1); font-size: 12px; font-weight: 600; }
      .dtp-check .hint { display: block; margin-top: 2px; color: var(--dsw-alias-label-secondary, #9297a5); font-size: 11px; font-weight: 400; line-height: 1.4; }
    ...
    `);
            ...
            function Card(props) {
                ...
                if (stage === 'backlog') {
                    ...
                    actions = h('div', { className: 'dtp-actions' }, h('span', { style: { fontSize: 11, color: '#c084fc', flexBasis: '100%' } }, h('span', { className: 'dtp-pulse', style: { background: '#a855f7' } }), '复核 agent 检查中' + (progress && progress.sessionId ? ' · ' + String(progress.sessionId).slice(0, 8) : '') + (progress ? ' · ' + Math.round((progress.elapsedMs || 0) / 1000) + 's' : '')), h('button', {
                        className: 'dtp-btn small ok',
                        onClick: onJumpSession,
                        disabled: !(progress && progress.sessionId),
                ...
                if ((stage === 'accepting' || stage === 'accepted') && req.reviewCount) {
                    ...
                    reviewBlock = h('div', { className: 'dtp-review ' + (passed ? 'ok' : 'bad'), title: '自动复核结论' }, h('span', { className: 'lab' }, passed ? '复核✓' : '复核!'), h('span', null, text));
                }
                ...
                return h('div', { className: 'dtp-card' }, h('div', { className: 'dtp-card-title' }, h('span', { className: 'dtp-pri dtp-pri-' + pri }, pri), ' ', req.title), h('div', { className: 'dtp-card-meta' }, h('span', null, req.id), h('span', null, '要素 ' + req.elementCount), h('span', null, '验收 ' + req.criterionCount), h('span', { title: req.autoReview === false ? '执行完成后不启动复核 agent' : '执行完成后自动启动复核 agent', style: { color: req.autoReview === false ? '#fbbf24' : '#a78bfa' } }, req.autoReview === false ? '免复核' : '自动复核'), req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'var(--dsw-alias-label-secondary, #9297a5)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '📁 ' + req.workdir) : h('span', { style: { color: '#fbbf24' } }, '⚠ 未绑定目录'), req.scheduledAt ? h('span', { title: '计划执行时间', style: { color: req.scheduledAt > Date.now() ? '#a78bfa' : '#34d399' } }, '⏱ ' + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h('span', { style: { color: '#fbbf24' } }, '返工 ' + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
            }
            // ── 新建 / 编辑表单 ──
            function RequirementForm(props) {
                ...
                }), h('span', { className: 'txt' }, '自动复核', h('span', { className: 'hint' }, autoReview ? '执行完成后启动复核 agent，再进入待验收。' : '执行完成后直接进入待验收，不启动复核 agent。')))), h('div', { className: 'dtp-modal-actions' }, h('button', { className: 'dtp-btn', onClick: onClose }, '取消'), h('button', { className: 'dtp-btn primary', onClick: save, disabled: busy }, busy ? '保存中…' : '保存'))), dirPickerOpen ? h(DirectoryPickerModal, {
                    initial: workdir,
