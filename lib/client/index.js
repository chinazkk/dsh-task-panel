...
const plugin = (() => {
    ...
    `);
            ...
            function Card(props) {
                ...
                return h('div', { className: 'dtp-card' }, h('div', { className: 'dtp-card-title' }, h('span', { className: 'dtp-pri dtp-pri-' + pri }, pri), ' ', req.title), h('div', { className: 'dtp-card-meta' }, h('span', null, req.id), h('span', null, '要素 ' + req.elementCount), h('span', null, '验收 ' + req.criterionCount), req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'var(--dsw-alias-label-secondary, #9297a5)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '📁 ' + req.workdir) : h('span', { style: { color: '#fbbf24' } }, '⚠ 未绑定目录'), req.scheduledAt ? h('span', { title: '计划执行时间', style: { color: req.scheduledAt > Date.now() ? '#a78bfa' : '#34d399' } }, '⏱ ' + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h('span', { style: { color: '#fbbf24' } }, '返工 ' + req.reworkCount) : null), progressBlock, deliverableBlock, actions);
            }
            // ── 新建 / 编辑表单 ──
