    ...
    `);
            ...
            function Card(props) {
                ...
                if (stage === 'backlog') {
                    ...
                    actions = h('div', { className: 'dtp-actions' }, h('span', { className: 'dtp-status-line', style: { color: '#e9dfc2' } }, h('span', { className: 'dtp-pulse' }), '子 agent 执行中' + (progress && progress.sessionId ? ' · ' + String(progress.sessionId).slice(0, 8) : '') + (progress ? ' · ' + Math.round((progress.elapsedMs || 0) / 1000) + 's' : '')), h('button', {
                    ...
                    actions = h('div', { className: 'dtp-actions' }, h('span', { className: 'dtp-status-line', style: { color: '#d9e3f2' } }, h('span', { className: 'dtp-pulse', style: { background: '#b8c4d6' } }), '复核 agent 检查中' + (progress && progress.sessionId ? ' · ' + String(progress.sessionId).slice(0, 8) : '') + (progress ? ' · ' + Math.round((progress.elapsedMs || 0) / 1000) + 's' : '')), h('button', {
                        className: 'dtp-btn small ok',
                        onClick: onJumpSession,
                        disabled: !(progress && progress.sessionId),
                ...
                return h('div', { className: 'dtp-card' }, h('div', { className: 'dtp-card-title' }, h('span', { className: 'dtp-pri dtp-pri-' + pri }, pri), ' ', req.title), h('div', { className: 'dtp-card-meta' }, h('span', null, req.id), h('span', null, '要素 ' + req.elementCount), h('span', null, '验收 ' + req.criterionCount), h('span', { title: req.autoReview === false ? '执行完成后不启动复核 agent' : '执行完成后自动启动复核 agent', style: { color: req.autoReview === false ? '#e9dfc2' : '#d9e3f2' } }, req.autoReview === false ? '免复核' : '自动复核'), req.contextAnchors && req.contextAnchors.length ? h('span', { title: '已关联历史会话', style: { color: '#d7eee4' } }, '会话 ' + req.contextAnchors.length) : null, req.workdir ? h('span', { title: '绑定工作目录', style: { color: 'rgba(203,213,225,.72)', maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, '📁 ' + req.workdir) : h('span', { style: { color: '#e9dfc2' } }, '⚠ 未绑定目录'), req.scheduledAt ? h('span', { title: '计划执行时间', style: { color: req.scheduledAt > Date.now() ? '#d9e3f2' : '#d7eee4' } }, '⏱ ' + formatScheduledAt(req.scheduledAt)) : null, req.reworkCount ? h('span', { style: { color: '#e9dfc2' } }, '返工 ' + req.reworkCount) : null), progressBlock, reviewBlock, deliverableBlock, actions);
            }
            // ── 新建 / 编辑表单 ──
            function RequirementForm(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', onClick: (e) => e.stopPropagation() }, h('h2', null, isEdit ? '编辑需求 ' + req.id : '新建需求'), !isEdit && !workdir
                    ? h('div', { className: 'dtp-field', style: { background: 'rgba(196,181,143,.12)', border: '1px solid rgba(196,181,143,.30)', borderRadius: 10, padding: '7px 10px', color: '#e9dfc2', fontSize: 11 } }, '⚠ 尚未绑定工作目录，子 agent 执行时无法确定落盘位置，请选择或填写「绑定工作目录」。')
            ...
            function DirectoryPickerModal(props) {
                ...
                return h('div', { className: 'dtp-modal-backdrop', onClick: onClose }, h('div', { className: 'dtp-modal', style: { width: 560 }, onClick: (e) => e.stopPropagation() }, h('h2', null, '选择工作目录'), h('div', { className: 'dtp-dirbrowser' }, h('div', { style: { display: 'flex', gap: 8, alignItems: 'center' } }, mode === 'native'
                    ...
                    : h('button', { className: 'dtp-btn', onClick: () => openDir(initial || undefined), disabled: loading }, '↻ 刷新'), h('button', { className: 'dtp-btn', onClick: () => openDir(undefined) }, '🏠 主目录')), error ? h('div', { style: { fontSize: 11, color: '#ead5d8', padding: '6px 10px', background: 'rgba(197,155,161,.10)', border: '1px solid rgba(197,155,161,.20)', borderRadius: 10 } }, '⚠ ' + error) : null, mode !== 'native'
